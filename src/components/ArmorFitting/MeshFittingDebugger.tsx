...
function Scene({ 
  ...
  // Real model refs - removed as they're now passed from parent
  // const avatarMeshRef = useRef<THREE.SkinnedMesh | null>(null)
  // const armorMeshRef = useRef<THREE.Mesh | null>(null)
  
  const groupRef = useRef<THREE.Group>(null)
  // const debugArrowGroupRef = useRef<THREE.Group>(null) - now passed from parent
  ...
  // Removed - now using ref from parent
  // const originalArmorGeometry = useRef<THREE.BufferGeometry | null>(null)
...
const AvatarArmorDemo: React.FC<{ 
  ...
  // Determine if we need to load separate animation files
  const needsAnimationFile = currentAnimation !== 'tpose' && isAnimationPlaying
  const animationPath = useMemo(() => {
    if (!needsAnimationFile) return null
    
    const match = avatarPath.match(/gdd-assets\/([^\/]+)\//);
    const avatarName = match ? match[1] : null;
    
    if (!avatarName) return null;
    
    if (currentAnimation === 'walking') {
      return `./gdd-assets/${avatarName}/animations/walking.glb`
    } else if (currentAnimation === 'running') {
      return `./gdd-assets/${avatarName}/animations/running.glb`
    }
    
    return null
  }, [avatarPath, currentAnimation, needsAnimationFile])
  
  // Load animation file if needed
  const animationGltf = useGLTF(animationPath || avatarPath)
  
  // Always use the base rigged model which contains all animations
  const avatar = useGLTF(avatarPath)
  const armor = useGLTF(armorPath)
...
const HelmetDemo: React.FC<{ 
  ...
  // Always use the base rigged model
  const avatar = useGLTF(avatarPath)
  const helmet = useGLTF(helmetPath)
