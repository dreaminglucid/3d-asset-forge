...
const RetextureModal: React.FC<RetextureModalProps> = ({
  ...
  const [customStyle, setCustomStyle] = useState<'realistic' | 'cartoon'>('realistic')
  ...
  const detectSuggestedCategories = (): string[] => {
    ...
    if (name.includes('sword') || name.includes('axe') || name.includes('mace') || 
        name.includes('helm') || name.includes('plate') || name.includes('shield')) {
      return ['metal']
    } else if (name.includes('bow') || name.includes('staff') || name.includes('wand')) {
      return ['wood']
    } else if (name.includes('leather') || name.includes('hide') || name.includes('chaps')) {
      return ['leather']
    }
  ...
  const allCategories = Array.from(new Set(allMaterials.map(p => p.category).filter(Boolean))) as string[]
  
  const filteredPresets = selectedCategories.length > 0 
    ? allMaterials.filter(p => p.category && selectedCategories.includes(p.category))
    : allMaterials
  ...
  const renderWorkflowSelection = () => (
    <ModalSection title="Choose Your Workflow">
      <div className="space-y-4">
            ...
            <CardContent className="flex items-center gap-6 p-6">
              ...
              <div className="flex-1">
                ...
                <CardDescription className="text-gray-600">
                  Quick generation using pre-configured material templates optimized for your asset type
                </CardDescription>
          ...
          >
            <CardContent className="flex items-center gap-6 p-6">
              ...
              <div className="flex-1">
                ...
                <CardDescription className="text-gray-600">
                  Describe your desired materials and style in natural language for custom generation
                </CardDescription>
              </div>
          ...
          >
            <CardContent className="flex items-center gap-6 p-6">
              ...
              <div className="flex-1">
                ...
                <CardDescription className="text-gray-600">
                  Upload or provide a reference image to match specific materials and textures
                </CardDescription>
              </div>
        ...
        {suggestedCategories.length > 0 && (
          <Card variant="default" className="bg-gradient-to-r from-primary-50 to-primary-100 border-primary-200">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-primary-700 mb-2">
                    Based on your asset "{asset.name}", we suggest these material categories:
                  </p>
  ...
  const renderPresetSelection = () => (
    <ModalSection title="Select Materials">
      <div className="space-y-6">
        {/* Category Selection */}
        <div>
          <h4 className="text-sm font-medium text-gray-700 mb-3">Material Categories</h4>
          <div className="flex flex-wrap gap-2">
      ...
      <div className="space-y-4">
        <div>
          ...
          <Textarea
            ...
            className="w-full"
          />
          <p className="text-sm text-text-tertiary mt-1">Be as specific as possible for best results</p>
        </div>
        ...
        <div>
          ...
          <Select
            value={customStyle}
            onChange={(e) => setCustomStyle(e.target.value as 'realistic' | 'cartoon')}
          >
    ...
    <ModalSection title="Reference Image">
      <div className="space-y-4">
        <div>
          ...
          />
          <p className="text-sm text-text-tertiary mt-1">Or upload a file below</p>
        </div>
  ...
  const renderFooter = () => {
    ...
    const canGenerate = workflowMode === 'presets' 
      ? selectedPresets.length > 0
      : workflowMode === 'custom'
  ...
  return (
    ...
    >
      ...
      <ModalBody>
        ...
        {status === 'idle' && workflowMode === 'presets' && renderPresetSelection()}
        {status === 'idle' && workflowMode === 'custom' && renderCustomPrompt()}
        {status === 'idle' && workflowMode === 'image' && renderImageReference()}
