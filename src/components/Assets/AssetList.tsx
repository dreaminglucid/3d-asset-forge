      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="p-2 space-y-1">
          {viewMode === 'grouped' ? (
            <>
              {/* Render grouped view */}
              {/* Base Items in Grouped View */}
              {viewMode === 'grouped' && assetGroups.groups.map((group, groupIndex) => (
                <div key={group.base.id} className="animate-scale-in-top" style={{ animationDelay: `${groupIndex * 30}ms` }}>
                  {/* Base Item */}
                  <div className={`group relative rounded-lg transition-all duration-200 ${
                    ...
                    <div className="flex items-center p-3">
                        ...
                        >
                          <ChevronRight 
                            ...
                            className={`text-text-tertiary transition-transform duration-200 ${
                              expandedGroups.has(group.base.id) ? 'rotate-90' : ''
                            }`} 
                          />
                        </button>
                      ) : (
                      ...
                      >
                        <div className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-200 group-hover:scale-105 ${
                          selectedAsset?.id === group.base.id
                    ...
                    <div className="ml-10 mt-0.5 space-y-0.5 animate-scale-in-top">
                      {group.variants.map((variant, variantIndex) => (
                        <div
                          ...
                          style={{
                            animationDelay: `${(groupIndex * 30) + (variantIndex * 10)}ms`
                          }}
                        >
                          <div className="flex items-center gap-3 p-2 pl-3">
                            <div className={`flex-shrink-0 w-8 h-8 rounded-md flex items-center justify-center transition-all duration-200 group-hover:scale-105 ${
                              selectedAsset?.id === variant.id
              ...
              {/* Standalone Items (No Base/Variants) */}
              {viewMode === 'grouped' && assetGroups.standalone.map((asset, index) => {
                return (
                  <div
                    key={asset.id}
                    ...
                    style={{
                      animationDelay: `${(assetGroups.groups.length * 30) + (index * 30)}ms`
                    }}
