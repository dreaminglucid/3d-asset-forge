...
export class MeshyService {
  ...
  /**
   * Start retexturing task using new Retexture API
   */
  async startRetextureTask(
    ...
    if (!style.textStylePrompt && !style.imageStyleUrl) {
      throw new Error('Either textStylePrompt or imageStyleUrl must be provided')
    }
    ...
    // Add input (either task ID or model URL)
    ...
    // Add style (either text prompt or image URL)
    ...
    const response = await retry(
      async () => {
        const res = await fetch(`${this.baseUrl}/openapi/v1/retexture`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${this.config.apiKey}`,
            'Content-Type': 'application/json'
        ...
        if (!res.ok) {
          const error = await res.text()
          throw new Error(`Meshy Retexture API error: ${res.status} - ${error}`)
        }
  ...
  async getRetextureTaskStatus(taskId: string): Promise<any> {
    ...
    console.log('🔍 Meshy Retexture Status Response:', JSON.stringify(response, null, 2))
    return response
  }
  ...
  async waitForRetextureCompletion(taskId: string, timeoutMs: number = 300000): Promise<any> {
    ...
    while (Date.now() - startTime < timeoutMs) {
      try {
        ...
        if (status.status === 'SUCCEEDED') {
          ...
          console.log(`     ⏳ Retexture Progress: ${status.progress || 0}% (${elapsed}s elapsed)`)
        }
    ...
    console.log(`     ❌ Retexture timeout after ${timeoutMs/1000} seconds`)
    return null
  }

  /**
   * Start retexturing task (legacy method)
   */
