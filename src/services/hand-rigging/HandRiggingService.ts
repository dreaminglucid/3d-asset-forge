...
export class HandRiggingService {
  ...
  async rigHands(
    ...
    console.log(`✅ Hand rigging complete in ${result.metadata.processingTime}ms`)
  ...
  private async processHand(
    ...
    if (!detection || detection.hands.length === 0) {
      ...
      console.log(`📸 Capture attempt ${i + 1} for ${wristInfo.side} hand...`)
      ...
      if (!detection || detection.hands.length === 0) {
        ...
        console.log(`📊 Model appears to have closed/fist hands or hands that are difficult to detect`)
        console.log(`💡 Tip: For best results, use models with open hands in T-pose or A-pose`)
    ...
    console.log(`✅ Hand detected with ${(hand.confidence * 100).toFixed(1)}% confidence`)
  ...
  private createHandBones(
    ...
    if (skeleton) {
      skeleton.update()
      console.log(`✅ Added bones to skeleton. Total bones: ${skeleton.bones.length}`)
    }
