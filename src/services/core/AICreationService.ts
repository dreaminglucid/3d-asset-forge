...
export class AICreationService extends EventEmitter {
  ...
  public meshyService: MeshyService  // Made public for retexturing
  ...
  // Getter for imageService
  get imageService(): ImageGenerationService {
  ...
  constructor(config: AICreationConfig) {
    ...
    // Initialize services
    this._imageService = new ImageGenerationService({
      apiKey: config.openai.apiKey,
      model: config.openai.model,
      imageServerBaseUrl: config.openai.imageServerBaseUrl
    })
  ...
  private async generateImage(result: GenerationResult): Promise<void> {
    ...
    try {
      ...
      if (cached) {
        ...
        // Generate new image
        const imageResult = await this._imageService.generateImage(
